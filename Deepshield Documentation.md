...
**Deterministic fallback auto-summary:** If both providers fail or exhaust their quotas, the system generates a structured auto-summary from raw forensic scores — for example: `"This image appears authentic with a deepfake probability of 28/100. EXIF metadata is consistent with camera origin. No significant GAN fingerprints detected."` Constructed from `verdict.authenticity_score`, `verdict.severity`, top `artifact_indicators`, and `exif.trust_adjustment`. The fallback produces 4 meaningful bullets covering verdict confidence, top artifact signal, EXIF trust status, and a practical guidance note. The `llm_summary` field is always populated with actionable content; generic "unavailable" messages are never returned. Returns `{paragraph, bullets: [str×4], model_used: "gemini" | "groq" | "auto-summary"}`.

**Payload optimization (`_build_llm_payload()`):** Constructs a minimal JSON payload — includes only verdict, key evidence, top sources, and pipeline context. Drops all heavy base64 fields (`heatmap_base64`, `ela_base64`, `boxes_base64`). Limits large arrays: OCR boxes capped at 5, video frames capped at 3, artifacts capped at 4, sources capped at 3. Gemini and Groq use JSON response mode with `max_output_tokens=600`. The prompt targets a 4–5 sentence paragraph and exactly 4 bullets written in plain-English audience-first language with practical guidance (e.g. "what this means for you" framing), replacing the previous compact technical format that produced truncated 2–3 sentence outputs. The response parser handles both structured JSON and plain-text responses — if the model returns plain text instead of the expected JSON schema, the parser wraps the raw text into the `paragraph` field and derives `bullets` from sentence splitting, eliminating "LLM explanation could not be parsed" errors in generated reports.
...
**`schemas/common.py`** — Shared Pydantic v2 models. `Verdict`: label, severity, authenticity_score, model_confidence, model_label, cached. `ArtifactIndicator`: type, severity, description, confidence. `ExifSummary`: per-field trust badges, trust_adjustment float. `VLMBreakdown`: 6 component scores with notes. `ProcessingSummary`: stages_completed list, total_duration_ms, models_used, calibrator_applied, heatmap_source, face_detector_used. All use `model_config = ConfigDict(protected_namespaces=())` to suppress `model_*` field name warnings.
