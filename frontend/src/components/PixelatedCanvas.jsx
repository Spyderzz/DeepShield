...
export default function PixelatedCanvas({
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      const onPointerLeave = () => {
        ...
        if (fadeOnLeave) activityTargetRef.current = 0; else { targetMouseRef.current.x = -9999; targetMouseRef.current.y = -9999; }
      };
      ...
      const animate = () => {
        ...
        if (backgroundColor) { ctx.fillStyle = backgroundColor; ctx.fillRect(0, 0, dims.width, dims.height); } else { ctx.clearRect(0, 0, dims.width, dims.height); }
    ...
    if (responsive) {
      ...
      return () => { isCancelled = true; window.removeEventListener('resize', onResize); if (img._cleanup) img._cleanup(); };
    }

    return () => { isCancelled = true; if (img._cleanup) img._cleanup(); };
  ...
  return (
    <canvas ref={canvasRef} className={className} aria-label="Pixelated rendering of source image" role="img" />
  );
}
